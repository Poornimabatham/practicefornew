...
export default class getgeofenceservice {
    ...
    }else
    {
      ...
      await Promise.all(
        latilongi.map(async (element: any) => {
          ...
          if (Status == 2) {
            const insertQuery = await Database.table(
      ...
      if (affected > 0) {
        ...
        const actionperformed =
          "<b>" +
          Name +
          "</b> has been created by <b>" +
          adminname +
          "</b> from<b> Attendance App  </b>";
        const activityby = "1";
        const affected1 = await Helper.ActivityMasterInsert(
          date,
          OrganizationId,
          adminname,
          activityby,
  ...
  public static async assignGeoFenceEmployee(data: any) {
    ...
    const query1: any = await Database.query()
      .from("EmployeeMaster")
      .select("area_assigned as areaIds")
      .where("Id", Empid)
      .andWhere("OrganizationId", orgId)
      .andWhere("area_assigned", "!=", 0);
    if (query1.length > 0) {
    ...
    if (affected2 > 2) {
      ...
      const actionperformed =
        "<b>" +
        geo_name +
        "</b> Geo-fence has been assigned to  <b>" +
        EmployeeName +
        "</b> by <b>" +
        adminname +
        "</b> from<b> Attendance App  </b>";
  ...
  /////////////// deleteGeoFence ///////////////
  public static async deleteGeoFence(getparam) {
    let Gid = getparam.area_assigned;
    let OrgId = getparam.OrganizationId;
    var result = {};

    var query = await Database.from("EmployeeMaster")
      .select("Id")
      .where("area_assigned", "LIKE", Gid)
      .andWhere("OrganizationId", OrgId);
    if (query.length == 0) {
      var query2 = await Database.from("AttendanceMaster")
        .select("Id")
        .where("areaId", Gid)
        .andWhere("OrganizationId", OrgId);

      if (query2.length == 0) {
        var query3 = await Database.from("ArchiveAttendanceMaster")
          .select("Id")
          .where("areaId", Gid)
          .andWhere("OrganizationId", OrgId);

        if (query3.length == 0) {
          var query4 = await Database.from("Geo_Settings")
            .where("Id", Gid)
            .andWhere("OrganizationId", OrgId)
            .delete();
