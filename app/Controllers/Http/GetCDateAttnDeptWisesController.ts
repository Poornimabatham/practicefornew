...
export default class GetCDateAttnDeptWisesController {
  private data = [];
  public async getCDateAttnDeptWise({
    request,
    response,
  }: HttpContextContract) {
    const getvalidData = await request.validate(
      GetCDateAttnDeptWiseValidator.GetCDateAttnDeptWise_getx
    );
    ...
    const getResponsefromService =
      await getCDateAttnDeptWiseService.getCDateAttnDeptWise(this.data);

    return response.json(getResponsefromService);
  }

  public async getCDateAttnDeptWise_getxCsv({
    request,
    response,
  }: HttpContextContract) {
    const getvalidData = await request.validate(
      GetCDateAttnDeptWiseValidator.GetCDateAttnDeptWise_getxCsv
    );
    ...
    const getResponsefromService =
      await getCDateAttnDeptWiseService.getCDateAttnDeptWiseCsv(this.data);
